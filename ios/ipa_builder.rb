...
module IpaBuilder
  APP_NAME    = "AWH"
  SCHEME      = "AWH"
  PROJECT_DIR = File.join(__dir__, APP_NAME)          # ios/AWH
  ...
  def unsigned? = ENV["AWH_UNSIGNED"] == "1"
  ...
  # export can never disagree. AWH_TEAM_ID overrides it for a one-off.
  def team_id
    id = ENV["AWH_TEAM_ID"] ||
         File.read(PROJECT_YML)[/^\s*DEVELOPMENT_TEAM:\s*"?([A-Z0-9]+)"?/, 1]
    die("no DEVELOPMENT_TEAM in #{PROJECT_YML} and no AWH_TEAM_ID set") unless id
    id
  end
  ...
  # 'com.awh.app' were found". Archiving does not hit this, because it is
  ...
  def build(manifest: nil)
    ...
    step "Archiving (xcodebuild)#{unsigned? ? ' [UNSIGNED]' : ''}"
    run(
      "xcodebuild " \
  ...
  # The pre-signing packaging path, kept for AWH_UNSIGNED=1: zip the
  ...
  def package_unsigned(archive, work)
    ...
    FileUtils.cp_r(app_path, payload)   # Payload/AWH.app
